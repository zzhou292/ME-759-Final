...
SCMDeformableTerrain::SCMDeformableTerrain(ChSystem *system,
  ...
  m_ground =
      chrono_types::make_shared<SCMDeformableSoil>(system, visualization_mesh);
  system->Add(m_ground);
}
...
float SCMDeformableTerrain::GetCoefficientFriction(
    const ChVector<> &loc) const {
...
void SCMDeformableTerrain::SetTexture(const std::string tex_file,
...
void SCMDeformableTerrain::WriteMesh(const std::string &filename) const {
  if (!m_ground->m_trimesh_shape) {
    std::cout << "SCMDeformableTerrain::WriteMesh  -- visualization mesh not "
                 "created.";
    return;
  }
...
void SCMDeformableTerrain::SetSoilParameters(
    ...
    double elastic_K,     // elastic stiffness K per unit area, [Pa/m] (must be
                          // larger than Kphi)
    double damping_R // vertical damping R per unit area [Pa.s/m] (proportional
                     // to vertical speed)
) {
...
void SCMDeformableTerrain::SetBulldozingParameters(
    ...
    int erosion_propagations // number of concentric vertex selections subject
                             // to erosion
) {
...
void SCMDeformableTerrain::SetPlotType(DataPlotType plot_type, double min_val,
                                       double max_val) {
...
void SCMDeformableTerrain::RegisterSoilParametersCallback(
    std::shared_ptr<SoilParametersCallback> cb) {
  m_ground->m_soil_fun = cb;
}

// Initialize the terrain as a flat grid.
void SCMDeformableTerrain::Initialize(double sizeX, double sizeY,
                                      double delta) {
...
void SCMDeformableTerrain::Initialize(const std::string &heightmap_file,
                                      double sizeX, double sizeY, double hMin,
                                      double hMax, double delta) {
  m_ground->Initialize(heightmap_file, sizeX, sizeY, hMin, hMax, delta);
}

// Get the heights of modified grid nodes.
std::vector<SCMDeformableTerrain::NodeLevel>
SCMDeformableTerrain::GetModifiedNodes(bool all_nodes) const {
  return m_ground->GetModifiedNodes(all_nodes);
}

// Modify the level of grid nodes from the given list.
void SCMDeformableTerrain::SetModifiedNodes(
    const std::vector<NodeLevel> &nodes) {
  m_ground->SetModifiedNodes(nodes);
}

// Return the current cumulative contact force on the specified body (due to
// interaction with the SCM terrain).
TerrainForce
SCMDeformableTerrain::GetContactForce(std::shared_ptr<ChBody> body) const {
...
// Return the number of nodes in the erosion domain at last step (bulldosing
// effects).
...
void SCMDeformableTerrain::PrintStepStatistics(std::ostream &os) const {
  ...
  os << "   Moving patches:          "
     << 1e3 * m_ground->m_timer_moving_patches() << std::endl;
  os << "   Ray casting:             " << 1e3 * m_ground->m_timer_ray_casting()
     << std::endl;
  os << "   Contact patches:         "
     << 1e3 * m_ground->m_timer_contact_patches() << std::endl;
  os << "   Contact forces:          "
     << 1e3 * m_ground->m_timer_contact_forces() << std::endl;
  os << "   Bulldozing:              " << 1e3 * m_ground->m_timer_bulldozing()
     << std::endl;
  os << "      Raise boundary:       "
     << 1e3 * m_ground->m_timer_bulldozing_boundary() << std::endl;
  os << "      Compute domain:       "
     << 1e3 * m_ground->m_timer_bulldozing_domain() << std::endl;
  os << "      Apply erosion:        "
     << 1e3 * m_ground->m_timer_bulldozing_erosion() << std::endl;
  os << "   Visualization:           "
     << 1e3 * m_ground->m_timer_visualization() << std::endl;

  os << " Counters:" << std::endl;
  os << "   Number ray casts:        " << m_ground->m_num_ray_casts
     << std::endl;
  os << "   Number ray hits:         " << m_ground->m_num_ray_hits << std::endl;
  os << "   Number contact patches:  " << m_ground->m_num_contact_patches
     << std::endl;
  os << "   Number erosion nodes:    " << m_ground->m_num_erosion_nodes
     << std::endl;
}
...
SCMContactableData::SCMContactableData(double area_ratio, double Mohr_cohesion,
                                       double Mohr_friction,
                                       double Janosi_shear)
    : area_ratio(area_ratio), Mohr_cohesion(Mohr_cohesion),
...
// Constructor.
SCMDeformableSoil::SCMDeformableSoil(ChSystem *system, bool visualization_mesh)
    : m_soil_fun(nullptr) {
  ...
  if (visualization_mesh) {
    ...
    m_trimesh_shape =
        std::shared_ptr<ChTriangleMeshShape>(new ChTriangleMeshShape);
...
void SCMDeformableSoil::Initialize(double sizeX, double sizeY, double delta) {
  ...
  m_nx = static_cast<int>(std::ceil(
      (sizeX / 2) / delta)); // half number of divisions in X direction
  m_ny = static_cast<int>(
      std::ceil((sizeY / 2) / delta)); // number of divisions in Y direction
  ...
  int n_verts =
      nvx * nvy; // total number of vertices for initial visualization trimesh
  int n_faces =
      2 * (2 * m_nx) *
      (2 * m_ny); // total number of faces for initial visualization trimesh
  ...
  for (int iy = 0; iy < nvy; iy++) {
    ...
    for (int ix = 0; ix < nvx; ix++) {
      ...
      normals[iv] =
          m_plane.TransformDirectionLocalToParent(ChVector<>(0, 0, 1));
...
void SCMDeformableSoil::Initialize(const std::string &heightmap_file,
  ...
  if (!hmap.ReadFromFile(heightmap_file, 1)) {
    std::cout << "STB error in reading height map file " << heightmap_file
              << std::endl;
  ...
  // Resample image and calculate interpolated gray levels and then map it to
  // the height range, with black corresponding to hMin and white corresponding
  // to hMax. Entry (0,0) corresponds to bottom-left grid vertex. Note that
  // pixels in the image start at top-left corner.
  ...
  for (int ix = 0; ix < nvx; ix++) {
    ...
    for (int iy = 0; iy < nvy; iy++) {
      double y =
          (2 * m_ny - iy) * dy_grid; // y location in image (in [0,1], 0 at top)
      ...
      m_heights(ix, iy) = (1 - ax) * (1 - ay) * g11 + (1 - ax) * ay * g12 +
  ...
  for (it = 0; it < n_faces; it++) {
    ...
    ChVector<> nrm =
        Vcross(vertices[idx_vertices[it][1]] - vertices[idx_vertices[it][0]],
...
void SCMDeformableSoil::SetupInitial() {
  // If no user-specified moving patches, create one that will encompass all
  // collision shapes in the system
  if (!m_moving_patch) {
    SCMDeformableSoil::MovingPatchInfo pinfo;
...
bool SCMDeformableSoil::CheckMeshBounds(const ChVector2<int> &loc) const {
  return loc.x() >= -m_nx && loc.x() <= m_nx && loc.y() >= -m_ny &&
         loc.y() <= m_ny;
}
...
std::vector<int>
SCMDeformableSoil::GetMeshFaceIndices(const ChVector2<int> &loc) {
  int i = loc.x();
  int j = loc.y();
...
// Get the initial undeformed terrain height (relative to the SCM plane) at the
// specified grid vertex.
...
// Get the initial undeformed terrain normal (relative to the SCM plane) at the
// specified grid node.
...
// Get the terrain height (relative to the SCM plane) at the specified grid
// vertex.
...
// Get the terrain normal (relative to the SCM plane) at the specified grid
// vertex.
...
void SCMDeformableSoil::UpdateMovingPatch(MovingPatchInfo &p,
  ...
  for (int j = 0; j < 8; j++) {
    ...
    ChVector<> c_body =
        p.m_center +
        p.m_hdims * ChVector<>(2.0 * ix - 1, 2.0 * iy - 1, 2.0 * iz - 1);
    // OOBB corner in absolute frame
    ChVector<> c_abs =
        p.m_body->GetFrame_REF_to_abs().TransformPointLocalToParent(c_body);
  ...
  // Calculate inverse of SCM normal expressed in body frame (for optimization
  // of ray-OBB test)
...
void SCMDeformableSoil::UpdateFixedPatch(MovingPatchInfo &p) {
  ...
  for (int j = 0; j < 8; j++) {
    ...
    ChVector<> c_abs = aabb_max * ChVector<>(ix, iy, iz) +
                       aabb_min * ChVector<>(1.0 - ix, 1.0 - iy, 1.0 - iz);
...
bool SCMDeformableSoil::RayOBBtest(const MovingPatchInfo &p,
                                   const ChVector<> &from,
                                   const ChVector<> &Z) {
...
void SCMDeformableSoil::ComputeInternalForces() {
  ...
  // Initialize list of modified visualization mesh vertices (use any
  // externally modified vertices)
  ...
  for (const auto &ij : m_modified_nodes) {
    ...
    if (m_trimesh_shape && CheckMeshBounds(ij)) {
      ...
      UpdateMeshVertexCoordinates(ij, iv,
                                  nr); // update vertex coordinates and color
...
#pragma omp critical(SCM_ray_casting)
      ...
      ChVector<> vertex_abs =
          m_plane.TransformPointLocalToParent(ChVector<>(x, y, z));
...
#pragma omp critical(SCM_ray_casting)
        {
          ...
          if (m_grid_map.find(ij) == m_grid_map.end()) {
            m_grid_map.insert(
                std::make_pair(ij, NodeRecord(z, z, GetInitNormal(ij))));
          }

          // Add to our map of hits to process
          HitRecord record = {mrayhit_result.hitModel->GetContactable(),
                              mrayhit_result.abs_hitPoint, -1};
  ...
  struct ContactPatchRecord {
    std::vector<ChVector2<>>
        points; // points in contact patch (in reference plane)
  ...
  // Loop through all hit nodes and determine to which contact patch they
  // belong. Use a queue-based flood-filling algorithm based on the neighbors of
  // each hit node.
  ...
  for (auto &h : hits) {
    ...
    while (!todo.empty()) {
      auto crt =
          hits.find(todo.front()); // Current hit node is first element in queue
      ...
      for (int k = 0; k < 4; k++) {
        ...
        patch.points.push_back(
            ChVector2<>(m_delta * nbr_ij.x(), m_delta * nbr_ij.y()));
  ...
  for (auto &h : hits) {
    ...
    const double &ca =
        nr.normal
            .z(); // cosine of angle between local normal and SCM plane vertical
    ...
    if (m_soil_fun) {
      double Mohr_friction;
      m_soil_fun->Set(hit_point_loc, Bekker_Kphi, Bekker_Kc, Bekker_n,
                      Mohr_cohesion, Mohr_friction, Janosi_shear, elastic_K,
                      damping_R);
      Mohr_mu = std::tan(Mohr_friction * CH_C_DEG_TO_RAD);
    }
    ...
    double p_hit_offset =
        ca * (nr.level_initial - nr.hit_level); // along local normal direction
    ...
    if (nr.sigma > nr.sigma_yield) {
      // Bekker formula
      nr.sigma = (contact_patches[patch_id].oob * Bekker_Kc + Bekker_Kphi) *
                 pow(nr.sinkage, Bekker_n);
      ...
      nr.step_plastic_flow =
          (nr.sinkage_plastic - old_sinkage_plastic) / GetSystem()->GetStep();
    }

    // Elastic sinkage (along local normal direction)
    nr.sinkage_elastic = nr.sinkage - nr.sinkage_plastic;

    // Add compressive speed-proportional damping (not clamped by pressure
    // yield)
    ////if (Vn < 0) {
    ...
    // If specified, combine properties for soil-contactable interaction and
    ...
    if (auto cprops = contactable->GetUserData<vehicle::SCMContactableData>()) {
      ...
      double c_tau =
          c_tau_max * (1.0 - exp(-(nr.kshear / cprops->Janosi_shear)));
    ...
    if (ChBody *rigidbody = dynamic_cast<ChBody *>(contactable)) {
      // [](){} Trick: no deletion for this shared ptr, since 'rigidbody' was
      // not a new ChBody() object, but an already used pointer because
      // mrayhit_result.hitModel->GetPhysicsItem() cannot return it as
      // shared_ptr, as needed by the ChLoadBodyForce:
      std::shared_ptr<ChBody> srigidbody(rigidbody, [](ChBody *) {});
      std::shared_ptr<ChLoadBodyForce> mload(
          new ChLoadBodyForce(srigidbody, Fn + Ft, false, point_abs, false));
      this->Add(mload);

      // Accumulate contact force for this rigid body.
      // The resultant force is assumed to be applied at the body COM.
      // All components of the generalized terrain force are expressed in the
      // global frame.
      ...
      if (itr == m_contact_forces.end()) {
        ...
        itr->second.moment +=
            Vcross(Vsub(point_abs, srigidbody->GetPos()), force);
      }
      ...
      mload->loader.SetApplication(0.5,
                                   0.5); //***TODO*** set UV, now just in middle
      this->Add(mload);
  ...
  if (m_bulldozing) {
    ...
    for (auto p : contact_patches) {
      ...
      // Calculate the displaced material from all touched nodes and identify
      // boundary
      ...
      for (const auto &ij : p.nodes) {      // for each node in contact patch
        ...
        tot_step_flow +=
            nr.step_plastic_flow;     //   accumulate displaced material
        for (int k = 0; k < 4; k++) { //   check each node neighbor
          ChVector2<int> nbr_ij =
              ij + neighbors4[k]; //     neighbor node coordinates
          ////if (!CheckMeshBounds(nbr_ij))                     //     if
          /// neighbor out of bounds /    continue; //       skip neighbor
          if (m_grid_map.find(nbr_ij) ==
              m_grid_map.end())        //     if neighbor not yet recorded
            p_boundary.insert(nbr_ij); //       set neighbor as boundary
          else if (m_grid_map.at(nbr_ij).sigma <=
                   0)                  //     if neighbor not touched
            p_boundary.insert(nbr_ij); //       set neighbor as boundary
        }
      }
      ...
      // Raise boundary (create a sharp spike which will be later smoothed out
      // with erosion)
      for (const auto &ij : p_boundary) {              // for each node in bndry
        ...
        if (m_grid_map.find(ij) == m_grid_map.end()) { //   if not yet recorded
          ...
          m_grid_map.insert(std::make_pair(
              ij, NodeRecord(z, z, n)));  //     add new node record
          m_modified_nodes.push_back(ij); //     mark as modified
        }                                 //
        auto &nr = m_grid_map.at(ij);     //   node record
    ...
    NodeSet erosion_front =
        boundary; // initialize erosion front to boundary nodes
    for (int i = 0; i < m_erosion_propagations; i++) {
      NodeSet front; // new erosion front
      for (const auto &ij :
           erosion_front) {           // for each node in current erosion front
        for (int k = 0; k < 4; k++) { // check each of its neighbors
          ChVector2<int> nbr_ij =
              ij + neighbors4[k]; //   neighbor node coordinates
          ////if (!CheckMeshBounds(nbr_ij))                       //   if out of
          /// bounds /    continue;                                       //
          /// ignore neighbor
          if (m_grid_map.find(nbr_ij) ==
              m_grid_map.end()) { //   if neighbor not yet recorded
            double z = GetInitHeight(
                nbr_ij); //     undeformed height at neighbor location
            const ChVector<> &n = GetInitNormal(
                nbr_ij);            //     terrain normal at neighbor location
            NodeRecord nr(z, z, n); //     create new record
            nr.erosion = true;      //     include in erosion domain
            m_grid_map.insert(
                std::make_pair(nbr_ij, nr));    //     add new node record
      ...
      erosion_domain.insert(front.begin(),
                            front.end()); // add current front to erosion domain
    ...
    for (int iter = 0; iter < m_erosion_iterations; iter++) {
      for (const auto &ij : erosion_domain) {
        ...
        for (int k = 0; k < 4; k++) {
          ...
          double diff = 0.5 * (nr.massremainder - nbr_nr.massremainder) /
                        4; //// TODO: rethink this!
          if (diff > 0) {
          ...
          // (3.2) Smoothing
          if (nbr_nr.sigma == 0) {
            double dy = (nr.level + nr.massremainder) -
                        (nbr_nr.level + nbr_nr.massremainder);
  ...
  if (m_trimesh_shape) {
    ...
    for (const auto &ij : m_modified_nodes) {
      ...
      modified_vertices.push_back(
          iv); // cache in list of modified mesh vertices
...
void SCMDeformableSoil::AddMaterialToNode(double amount, NodeRecord &nr) {
  if (amount >
      nr.hit_level - nr.level) { //   if not possible to assign all mass
    nr.massremainder +=
        amount -
        (nr.hit_level - nr.level);    //     material to be further propagated
...
void SCMDeformableSoil::RemoveMaterialFromNode(double amount, NodeRecord &nr) {
  if (nr.massremainder > amount) { // if too much remainder material
    ...
    /*amount = 0;*/                //   ???
  } else if (nr.massremainder < amount &&
             nr.massremainder > 0) { // if not enough remainder material
...
void SCMDeformableSoil::UpdateMeshVertexCoordinates(const ChVector2<int> ij,
                                                    int iv,
                                                    const NodeRecord &nr) {
  auto &trimesh = *m_trimesh_shape->GetMesh();
  std::vector<ChVector<>> &vertices = trimesh.getCoordsVertices();
  ...
  vertices[iv] = m_plane.TransformPointLocalToParent(
      ChVector<>(ij.x() * m_delta, ij.y() * m_delta, nr.level));
  ...
  if (m_plot_type != SCMDeformableTerrain::PLOT_NONE) {
    ...
    case SCMDeformableTerrain::PLOT_LEVEL_INITIAL:
      mcolor = ChColor::ComputeFalseColor(nr.level_initial, m_plot_v_min,
                                          m_plot_v_max);
      break;
    case SCMDeformableTerrain::PLOT_SINKAGE:
      mcolor =
          ChColor::ComputeFalseColor(nr.sinkage, m_plot_v_min, m_plot_v_max);
      break;
    case SCMDeformableTerrain::PLOT_SINKAGE_ELASTIC:
      mcolor = ChColor::ComputeFalseColor(nr.sinkage_elastic, m_plot_v_min,
                                          m_plot_v_max);
      break;
    case SCMDeformableTerrain::PLOT_SINKAGE_PLASTIC:
      mcolor = ChColor::ComputeFalseColor(nr.sinkage_plastic, m_plot_v_min,
                                          m_plot_v_max);
      break;
    case SCMDeformableTerrain::PLOT_STEP_PLASTIC_FLOW:
      mcolor = ChColor::ComputeFalseColor(nr.step_plastic_flow, m_plot_v_min,
                                          m_plot_v_max);
      break;
    case SCMDeformableTerrain::PLOT_K_JANOSI:
      mcolor =
          ChColor::ComputeFalseColor(nr.kshear, m_plot_v_min, m_plot_v_max);
      break;
    ...
    case SCMDeformableTerrain::PLOT_PRESSURE_YELD:
      mcolor = ChColor::ComputeFalseColor(nr.sigma_yield, m_plot_v_min,
                                          m_plot_v_max);
      break;
    ...
    case SCMDeformableTerrain::PLOT_MASSREMAINDER:
      mcolor = ChColor::ComputeFalseColor(nr.massremainder, m_plot_v_min,
                                          m_plot_v_max);
      break;
...
void SCMDeformableSoil::UpdateMeshVertexNormal(const ChVector2<int> ij,
                                               int iv) {
  auto &trimesh = *m_trimesh_shape->GetMesh();
  std::vector<ChVector<>> &vertices = trimesh.getCoordsVertices();
  std::vector<ChVector<>> &normals = trimesh.getCoordsNormals();
  ...
  for (auto f : faces) {
    ChVector<> nrm =
        Vcross(vertices[idx_normals[f][1]] - vertices[idx_normals[f][0]],
...
// Get the heights of modified grid nodes.
std::vector<SCMDeformableTerrain::NodeLevel>
SCMDeformableSoil::GetModifiedNodes(bool all_nodes) const {
...
// Modify the level of grid nodes from the given list.
// NOTE: We set only the level of the specified nodes and none of the other soil
// properties.
//       As such, some plot types may be incorrect at these nodes.
void SCMDeformableSoil::SetModifiedNodes(
    const std::vector<SCMDeformableTerrain::NodeLevel> &nodes) {
  for (const auto &n : nodes) {
    // Modify existing entry in grid map or insert new one
    m_grid_map[n.first] = SCMDeformableSoil::NodeRecord(n.second, n.second,
                                                        GetInitNormal(n.first));
  }
